...
{
    public interface ISPIDevice : ISPIPeripheral
    {
        void DeviceSelected();
    }
    ...
    public class SPIMultiplexer : SimpleContainer<ISPIPeripheral>, IGPIOReceiver, ISPIPeripheral
    {
        ...
        private void SelectCutterDeviceByAddress(int deviceAddress)
        {
            if(!TryGetByAddress(deviceAddress, out var device))
            {
                this.Log(LogLevel.Warning, "Tried to select device 0x{0:X}, but it's not connected", deviceAddress);
                return;
            }

            this.Log(LogLevel.Noisy, "Selecting device 0x{0:X}", deviceAddress);

            if(device is ISPIDevice spiDevice)
            {
                spiDevice.DeviceSelected();
            }else{
                this.Log(LogLevel.Noisy, "Device 0x{0:X} does not implement ISPIDevice interface, so it won't be notified about selection", deviceAddress);
            }
        }
        ...
        private void UpdateChipSelectState()
        {
            foreach(var state in inputState)
            {
                ...
                if(active)
                {
                    ...
                    SelectCutterDeviceByAddress(number);
                }
                else
                {
